...
export const config: Options.Testrunner = {
    ...
    logLevel: process.env.DEBUG.toUpperCase() === 'YES' ? 'info' : 'error',
    //
     ...
     */
     beforeScenario: async function (world, context) {
        ...
        const nameArr = await world.pickle.name.split(':');
        if(nameArr.length > 0){
            global.testId = nameArr[0];
        }
     },
    /**
     *
     ...
     */
     afterFeature: function (uri, feature) {
         // allure details for the report
        allure.addEnvironment('Environment: ', process.env.ENVIRONMENT);
     },
